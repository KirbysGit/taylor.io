...
### Category policy
Use `skillCategoryPolicy` when present. **Stable categories** (for concrete tools such as Languages, Frameworks, Data Tools, Databases, Cloud, Tools) should usually keep their category labels; reorder rows and demote before changing names. **Flexible categories** (Focus Areas, Strengths, Competencies, General Skills, similar) are tailorable positioning space: you may rename the category and rewrite row names into role-shaped strengths when the resume proves them. If there are no `flexibleCategories`, do not invent one. Never add unsupported skills or turn a hard tool row into a vague soft skill.
...
summaryDecision: required object with `action`, `confidence`, `reason`, and `evidence`. `action` must be `show`, `hide`, or `keep`. Show the summary only when it earns the space by repositioning the candidate, connecting scattered proof, explaining a pivot, or placing a role-specific thesis the body does not immediately convey. Hide it when the draft is tight/direct-fit and the summary would repeat obvious evidence. Keep when the evidence is mixed or no structural change is justified.
...
**categoryStrategy** (bucket-level; optional): use this for **merge / rename / collapse** of vague skill-like groupings (“Focus Areas,” “Strengths,” “Competencies,” “General Skills,” etc.)—**not** duplicate the whole ordering job of skillsStrategy; pass B respects both. `[]` when N/A. Stable tool categories like Languages, Frameworks, Data Tools, Databases, Cloud, and Tools are mostly reorder space, not creative naming space.
...
Layout is part of tailoring, but subordinate to content. Use `summaryDecision`, `layoutStrategy`, `layoutSectionOrder`, `layoutSectionVisibility`, and `layoutRationale` to decide section shape. Summary visibility should be role-driven by `summaryDecision`: hide it when it repeats obvious evidence in a tight direct-fit draft; show it when it helps reposition the candidate, clarify a transition, or foreground a role-specific thesis.
...
summaryDecision: action show/hide/keep, confidence high/medium/low, short reason, 0–3 evidence strings.
