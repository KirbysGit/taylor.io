# Project-forward layout: single body flow with projects promoted after summary.
...
def build_docx_project_forward_document(resumeData, style, stylePreferences=None, templateSlug="project-forward", docxMaxPages=None):
    # Initialize Docx.
    doc = Document()
    ...
    # Add header first; project-forward templates can use header_alignment tokens.
    ...
    # Projects are promoted immediately after the summary, then the rest follows a stable order.
    for sectionKey in project_forward_body_order(resumeData):
