...
function PhLine(props) {
	return (
		<span
			aria-hidden
			className={`block h-1 shrink-0 rounded-full bg-neutral-400/[0.35] ${props.className ?? ''}`}
		/>
	)
}
...
function BulletLine(props) {
	const dot =
		props.accent === true ? (
			<span className="size-1.5 shrink-0 rounded-full bg-brand-pink" aria-hidden />
		) : (
			<span className="size-1 shrink-0 rounded-full bg-neutral-300" aria-hidden />
		)
	const lineClass = props.lineClass ?? 'w-full'
	return (
		<div className="flex min-w-0 items-center gap-2">
			{dot}
			<PhLine className={`min-w-0 ${lineClass}`} />
		</div>
	)
}

/** Two ragged “skill chip” rows from short rails — reads as keywords without literal pills. */
function SkillPlaceholderRows() {
	return (
		<div className="mt-1.5 space-y-2">
			<div className="flex min-w-0 items-center gap-2">
				<PhLine className="w-[30%] shrink-0" />
				<PhLine className="w-[15%] shrink-0" />
				<PhLine className="w-[25%] shrink-0" />
				<PhLine className="w-[10%] shrink-0" />
			</div>
			<div className="flex min-w-0 items-center gap-2">
				<PhLine className="w-[12%] shrink-0" />
				<PhLine className="w-[28%] shrink-0" />
				<PhLine className="w-[18%] shrink-0" />
				<PhLine className="w-[22%] shrink-0" />
				<PhLine className="w-[8%] shrink-0" />
			</div>
		</div>
	)
}

export default function HowItWorksGenerateResumeCard() {
	return (
		<div className="relative flex h-full min-h-0 flex-col overflow-visible text-left" aria-hidden="true">
			{/* Soft outer cushion + pink glow so this step reads as “output” vs the flat profile/build mocks. */}
			<div className="relative flex min-h-0 flex-1 flex-col overflow-visible rounded-xl bg-brand-pink-lighter/35 p-1 shadow-[inset_0_0_0_1px_rgba(214,86,86,0.12)]">

				<div className="relative flex min-h-0 flex-1 flex-col overflow-visible rounded-lg border border-brand-pink/15 bg-[#fdfcfa] px-2.5 pb-5 pt-2 shadow-[0_10px_28px_-8px_rgba(214,86,86,0.28)]">
					...
					<span
						className="pointer-events-none absolute left-0 top-0 z-30 inline-flex origin-center -translate-x-[10%] -translate-y-[38%] items-center gap-1.5 rounded-full border border-emerald-400/60 bg-emerald-50/98 px-2 py-1 shadow-[0_3px_10px_rgba(15,118,110,0.22)] ring-[2px] ring-white/95"
						aria-hidden
					>
					...
					<div className="relative z-10 mt-2 flex min-h-0 flex-1 flex-col space-y-2 overflow-hidden">
						<div>
							...
							<div className="mt-1.5 space-y-2">
								<PhLine className="w-full" />
								<PhLine className="w-[92%]" />
								<PhLine className="w-[78%]" />
							</div>
						</div>
						...
						<div>
							...
							<div className="mt-1.5 space-y-2">
								<BulletLine accent lineClass="w-full" />
								<BulletLine lineClass="w-[88%]" />
								<BulletLine lineClass="w-[80%]" />
							</div>
						</div>
						...
						<div>
							...
							<div className="mt-1.5">
								<BulletLine accent lineClass="w-[90%]" />
							</div>
						</div>
						...
						<div>
							...
							<div className="mt-1.5 space-y-2">
								<BulletLine accent lineClass="w-[86%]" />
								<BulletLine lineClass="w-[82%]" />
								<BulletLine lineClass="w-[82%]" />
							</div>
						</div>
